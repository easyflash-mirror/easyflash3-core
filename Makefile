...
vhdl			:=
verilog			:=
vhdl			+= $(src_dir)/ef3.vhdl
vhdl			+= $(src_dir)/exp_bus_ctrl.vhdl
vhdl			+= $(src_dir)/reset_generator.vhdl
vhdl			+= $(src_dir)/freezer.vhdl
vhdl			+= $(src_dir)/cart_easyflash.vhdl
vhdl			+= $(src_dir)/cart_io2ram.vhdl
vhdl			+= $(src_dir)/cart_kernal.vhdl
vhdl			+= $(src_dir)/cart_ar.vhdl
vhdl			+= $(src_dir)/cart_ss5.vhdl
vhdl			+= $(src_dir)/cart_usb.vhdl
...
################################################################################
$(project): $(MAKEFILE_LIST)
	rm -f $@
	$(foreach file,$(verilog),echo "verilog work \"../$(file)\"" >> $@;)
	$(foreach file,$(vhdl),echo "vhdl work \"$(file)\"" >> $@;)

################################################################################
.PHONY: netlist
netlist: $(netlist)
$(netlist): $(xst_cfg) $(vhdl) $(verilog) $(project) | $(dirs)
